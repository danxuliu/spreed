...
class Manager {
	...
	public function createRoomObject(array $row) {
		...
		return new Room($this, $this->db, $this->secureRandom, $this->dispatcher, $this->hasher, (int) $row['id'], (int) $row['type'], $row['token'], $row['name'], $row['password'], (int) $row['active_guests'], $activeSince, $row['object_type'], $row['object_id']);
	}
