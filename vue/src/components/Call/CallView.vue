...
export default {
	...
	computed: {
		...
		reversedCallParticipantModels() {
			return this.callParticipantCollection.callParticipantModels.slice().reverse()
		},

		remoteParticipantsCount() {
			return this.callParticipantCollection.callParticipantModels.length
		},

		callParticipantModelsWithScreen() {
			return this.callParticipantCollection.callParticipantModels.filter(callParticipantModel => callParticipantModel.attributes.screen)
		},
	...
	watch: {
		...
		'darkBackground': function(darkBackground) {
			this.$emit('hasDarkBackground', darkBackground)
		},
		...
		'callParticipantCollection.callParticipantModels': function(models) {
			let addedModels = models.filter(model => !this.sharedDatas[model.attributes.peerId])
			let removedModelIds = Object.keys(this.sharedDatas).filter(sharedDataId => models.find(model => model.attributes.peerId === sharedDataId) === undefined)
	...
	methods: {
		...
		_setSpeaking(peerId, speaking) {
			...
			if (index === 0) {
				...
				var firstInactiveSpeakerIndex = this.speakers.findIndex(speaker => !speaker.active)
