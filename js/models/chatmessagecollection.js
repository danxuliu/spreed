...
(function(OCA, OC, Backbone) {
	...
	var ChatMessageCollection = Backbone.Collection.extend({
		...
		receiveMessages: function() {
			...
			var fetchParameters = {};
			fetchParameters.lookIntoFuture = 1;
			fetchParameters.lastKnownMessageId = this.lastKnownMessageId;
		...
		_successfulFetch: function(collection, response) {
			// TODO use the header instead if provided
			if (response && response.ocs.meta.statuscode == 200) {
				this.lastKnownMessageId = response.ocs.data[response.ocs.data.length - 1].id;
			}
